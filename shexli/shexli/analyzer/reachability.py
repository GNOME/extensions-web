...
ENTRYPOINT_CONTEXTS = {
    "extension.js": JSContext.EXTENSION,
    "prefs.js": JSContext.PREFERENCES,
}
...
def reachable_js_contexts(
...
) -> dict[Path, set[JSContext]]:
    root = root.resolve()
    js_files = [path.resolve() for path in js_files]
    js_file_set = set(js_files)
    contexts: dict[Path, set[JSContext]] = {}
    pending: list[tuple[Path, JSContext]] = []
    seen: set[tuple[Path, JSContext]] = set()
    ...
    while pending:
        ...
        for launched_path in launched_local_modules_in_program(
            ...
            pending.append((launched_path, JSContext.SCRIPT))
