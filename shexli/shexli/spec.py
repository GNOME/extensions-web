...
RULES = {
    ...
    for rule in (
        ...
        RuleSpec(
            ...
            title=(
                "metadata session-modes must be omitted when only user mode "
                + "is declared"
            ),
            severity="warning",
            source_url=f"{GUIDELINES_URL}#metadata-json-must-be-well-formed",
        ...
        RuleSpec(
            ...
            title=(
                "extensions using unlock-dialog must document it in "
                + "disable() comments"
            ),
            severity="warning",
            ...
            rationale=(
                "unlock-dialog requires reviewer justification and extra caution."
            ),
        ),
        RuleSpec(
        ...
        RuleSpec(
            ...
            static_checkable=True,
            detection_strategy=(
                "import graph reachability from extension.js and prefs.js"
            ),
            rationale=(
        ...
        RuleSpec(
            ...
            title=(
                "Clutter, Meta, St and Shell must not be imported in prefs "
                "process files"
            ),
            severity="error",
        ...
        RuleSpec(
            ...
            title=(
                "extension files should not contain excessive ungated "
                + "console logging"
            ),
            severity="warning",
            source_url=f"{GUIDELINES_URL}#no-excessive-logging",
        ...
        RuleSpec(
            ...
            title=(
                "extensions targeting GNOME 49 must not use removed Clutter "
                "action classes"
            ),
            severity="error",
            source_url=(
