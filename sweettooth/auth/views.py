...
class EmailChangeConfirmView(TemplateView):
    ...
    def get(self, request: HttpRequest, user_id: int, signature: str, *args, **kwargs) -> HttpResponse:
        user = User.objects.get(pk=user_id)
        message = _("Your email address updated successfully")
        try:
            data = TimestampSigner(salt=f"{self.SALT}:{user.password}").unsign_object(signature, max_age=self.MAX_AGE)
...
class SettingsView(LoginRequiredMixin, TemplateView):
    ...
    @staticmethod
    def _send_settings_mail(subject_template: str, message_template: str, to: str, data: dict[str, Any]):
        subject = render_to_string(f'profile/{subject_template}.txt', data) \
            .strip() \
            .replace('\n', '') \
            .replace('\r', '')
                ...
                'cancel_email_change_url': request.build_absolute_uri(
                    reverse(
                        ...
                        kwargs={
                            ...
                            'token': TimestampSigner(salt=EmainChangeCancelTokenGenerator.SALT).sign_object({
                                'user_id': user.pk,
                                'email': user.email,
                            }),
                        },
            ...
            email,
            {
                ...
                'confirm_email_change_url': request.build_absolute_uri(
                    reverse(
                        ...
                        kwargs={
                            'user_id': user.pk,
                            'signature': TimestampSigner(salt=f"{EmailChangeConfirmView.SALT}:{user.password}").sign_object({
                                'user_id': user.pk,
                                'email': email,
                            })
                        },
    ...
    def _delete_account_post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        self.extra_context = self._initial_context(request.user) | {'delete_account_form': form} | self._schedule_delete_context(schedule_delete)
        context = self.get_context_data(**kwargs)
        return self.render_to_response(context)
