...
class Command(BaseCommand):
    ...
    def _with_extensions(queryset):
        return queryset.annotate(extensions_count=Count('extension')).filter(extensions_count__gt=0)

    @transaction.atomic
    def handle(self, *args, **options):
        maintainer_wanted, _ = User.objects.get_or_create(username=settings.MAINTAINER_WANTED_USERNAME)
        now = datetime.datetime.now()

        scheduled_users = User.objects \
        ...
        inactive_users = User.objects \
            ...
            .filter(date_joined__lte=now - datetime.timedelta(days=5))

        users_with_extensions = (self._with_extensions(scheduled_users) | self._with_extensions(inactive_users))
